...
warnings.formatwarning = (
    lambda msg, *args, **kwargs: str(msg) + "\n"
        ...
        **deprecated_options
        ...
        if type(self.masker) is maskers.Independent:
            ...
            raise InvalidMaskerError(
                "Unsupported masker type: %s!" % str(type(self.masker))
            )
        ...
        if "feature_dependence" in deprecated_options:
            ...
            if dep_val == "independent" and feature_perturbation == "interventional":
                warnings.warn(
                    'feature_dependence = "independent" has been renamed to feature_perturbation'
            ...
            if (
                dep_val == "tree_path_dependent"
                and feature_perturbation == "interventional"
            ):
                raise ValueError(
                    "The feature_dependence option has been renamed to feature_perturbation! "
        ...
        if feature_perturbation == "independent":
            raise InvalidFeaturePerturbationError(
                'feature_perturbation = "independent" is not a valid option value, please use '
        ...
        if feature_perturbation not in feature_perturbation_codes:
            raise InvalidFeaturePerturbationError(
                "Invalid feature_perturbation option!"
            )
        ...
        if (
            ...
            and self.model.model_output != "raw"
        ):
            import xgboost
    ...
    def _validate_inputs(self, X, y, tree_limit, check_additivity):
        ...
        if self.model.model_output == "log_loss":
            assert y is not None, (
                "Both samples and labels must be provided when model_output = "
                '"log_loss" (i.e. `explainer.shap_values(X, y)`)!'
            )
            assert X.shape[0] == len(y), (
                "The number of labels (%d) does not match the number of samples to explain ("
                "%d)!" % (len(y), X.shape[0])
            )
...
class TreeEnsemble:
    ...
    def __init__(self, model, data=None, data_missing=None, model_output=None):
        ...
        if type(model) is dict and "trees" in model:
            ...
            type(model) is list and type(model[0]) == SingleTree
            ...
            if safe_isinstance(
                ...
                )  # with two classes the trees only model the second class. # pylint: disable=no-member
                self.tree_output = "log_odds"
            else:
                assert False, "Unsupported init model type: " + str(type(model.init_))
            ...
            if safe_isinstance(
                ...
                model, "pyspark.ml.regression.RandomForestRegressionModel"
            ):
            ...
            # Spark GBT, create 1 weighted (learning rate) tree per sub-model
                ...
                model, "pyspark.ml.classification.GBTClassificationModel"
            ...
            # Spark Basic model (single tree)
                ...
                model, "pyspark.ml.classification.DecisionTreeClassificationModel"
            ) or safe_isinstance(
            ...
            if hasattr(model, "best_iteration"):
                trees_per_iteration = (
                    xgb_loader.num_class if xgb_loader.num_class > 0 else 1
                )
                self.tree_limit = (
            ...
            safe_isinstance(model, "ngboost.ngboost.NGBoost")
            ...
            self.trees = [
                SingleTree(e.tree_, scaling=s, data=data, data_missing=data_missing)
        ...
        if self.trees is not None and self.trees:
            ...
            if self.num_stacked_models > 1:
                assert (
                    len(self.trees) % self.num_stacked_models == 0
                ), "Only stacked models with equal numbers of trees are supported!"
                assert (
                    self.trees[0].values.shape[1] == 1
            ...
            for i in range(num_trees):
                ...
                self.children_right[
                    i, : len(self.trees[i].children_right)
                ] = self.trees[i].children_right
                self.children_default[
                    i, : len(self.trees[i].children_default)
    ...
    def get_transform(self):
        ...
        if self.model_output == "raw":
            ...
            self.model_output == "probability"
            or self.model_output == "probability_doubled"
        ):
            ...
            raise ValueError(
                "Unrecognized model_output parameter value: %s! If model.%s is a valid function open a github issue to ask that this method be supported. If you want 'predict_proba' just use 'probability' for now."
...
class SingleTree:
    ...
    def __init__(
        ...
        # deprecated dictionary support (with sklearn singlular style "feature" and "value" names)
            ...
            tree, "pyspark.ml.classification.DecisionTreeClassificationModel"
                ...
                if "split_index" in vertex.keys():
                    if vertex["split_index"] not in visited:
                        if "split_index" in vertex["left_child"].keys():
                            self.children_left[vertex["split_index"]] = vertex[
                                "left_child"
                            ]["split_index"]
                        else:
                            self.children_left[vertex["split_index"]] = (
                                vertex["left_child"]["leaf_index"] + num_parents
                            )
                        if "split_index" in vertex["right_child"].keys():
                            self.children_right[vertex["split_index"]] = vertex[
                                "right_child"
                            ]["split_index"]
                        else:
                            self.children_right[vertex["split_index"]] = (
                        ...
                        self.node_sample_weight[vertex["split_index"]] = vertex[
                            "internal_count"
                        ]
                    ...
                    self.values[vertex["leaf_index"] + num_parents] = [
                        vertex["leaf_value"]
                    ]
                    self.node_sample_weight[
                        vertex["leaf_index"] + num_parents
                    ] = vertex["leaf_count"]
            self.values = np.asarray(self.values)
    ...
    def __init__(self, xgb_model):
        ...
        if version.parse(xgboost.__version__).major >= 1:
            if self.name_obj in ["binary:logistic", "reg:logistic"]:
                self.base_score = scipy.special.logit(
                    self.base_score
        ...
        for i in range(self.num_trees):
            for j in range(self.num_nodes[i]):
                ...
                if self.node_cleft[i][j] >= 0:
                    ...
                    self.thresholds[i, j] = np.nextafter(
                        self.node_info[i][j], -np.float32(np.inf)
                    )
                else:
